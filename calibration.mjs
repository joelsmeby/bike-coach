// OpenLog Artemis product axes with the USB connector pointing forward and the
// microSD-card side facing down. At rest the accelerometer reports -Z for up.
export const DEVICE_AXES_VERSION = 1;
export const DEVICE_FORWARD = [0, -1, 0];
export const DEVICE_LEFT = [-1, 0, 0];
export const DEVICE_UP = [0, 0, -1];
...
function frameForUp(measuredUp,leftReference=DEVICE_LEFT){
...
export function calibrate(samples, standalone=false) {
  ...
  return {ok:true,version:4,deviceAxesVersion:DEVICE_AXES_VERSION,windowSeconds:standalone?[time(rows[0]),time(rows.at(-1))]:[1,5],sampleCount:rows.length,...frame,bias,
    gravityMg:g,accelRmsMg:rms(ad),gyroRmsDps:rms(gd),diagnostics};
}
export function validCalibration(c){
  if(!c?.ok || ![1,2,3,4].includes(c.version))return false;
  ...
  if((c.version===3||(c.version===4&&c.levelReferenced))&&(!c.levelReferenced||!Array.isArray(c.measuredUp)||c.measuredUp.length!==3||!c.measuredUp.every(Number.isFinite)||Math.abs(norm(c.measuredUp)-1)>=1e-5||dot(c.measuredUp,c.up)<.99999||!validFrame(c.levelFrame)))return false;
  if(c.version===4&&c.deviceAxesVersion!==DEVICE_AXES_VERSION)return false;
  return validFrame(c)&&norm(c.bias)<=3&&
    axes.every(v=>Math.abs(norm(v)-1)<1e-5);
}
export function usesDeviceAxes(c){return validCalibration(c)&&c.version===4&&c.deviceAxesVersion===DEVICE_AXES_VERSION;}
...
export function setLevelReference(c){
  ...
  const measuredUp=c.levelReferenced&&Array.isArray(c.measuredUp)?c.measuredUp:c.up;
  const levelFrame={forward:[...c.forward],left:[...c.left],up:[...c.up]};
  const current=usesDeviceAxes(c)||c.version===4;
  return {...structuredClone(c),version:current?4:3,...(current?{deviceAxesVersion:DEVICE_AXES_VERSION}:{}),levelReferenced:true,levelFrame,measuredUp:[...measuredUp]};
}
export function useLevelReference(c,reference){
  if(!validCalibration(c)||!hasLevelReference(reference))return null;
  const measuredUp=c.levelReferenced&&Array.isArray(c.measuredUp)?c.measuredUp:c.up,frame=frameForUp(measuredUp,usesDeviceAxes(c)?DEVICE_LEFT:reference.levelFrame.left);
  if(!frame)return null;
  const levelFrame={forward:[...reference.levelFrame.forward],left:[...reference.levelFrame.left],up:[...reference.levelFrame.up]};
  const current=usesDeviceAxes(c);
  return {...structuredClone(c),version:current?4:3,...(current?{deviceAxesVersion:DEVICE_AXES_VERSION}:{}),levelReferenced:true,...frame,levelFrame,measuredUp:[...measuredUp]};
}
